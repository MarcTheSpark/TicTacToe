...
games = extrapolate_all_games([start_with_center], prune_symmetrical=True, skill=4)
...
move_sequences = itertools.permutations((1, 2, 3, 4, 5, 6, 7, 8, 9))
...
def draw_move_lines(screen, move_dict, origin=None):
    ...
    if origin:
        for x in move_dict:
            ...
            if origin + endpoint not in lines_done:
                lines_done.add(origin + endpoint)
                pygame.draw.line(screen, (255, 255, 255), origin, endpoint, 2)
                pygame.display.flip()
                time.sleep(0.01)
            draw_move_lines(screen, move_dict[x], endpoint)
        if len(move_dict) == 0:
            pygame.draw.circle(screen, (255, 255, 255), (origin[0], origin[1]), 6)
    else:
        for x in move_dict:
...
def main():
    ...
    # Set up the display
    screen = pygame.display.set_mode(SCREEN_DIM, pygame.FULLSCREEN | pygame.HWSURFACE | pygame.DOUBLEBUF)
